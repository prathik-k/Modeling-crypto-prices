...
class GBM_attention(GBM_base):
    ...
    def __init__(self,crypto,pred_type,hist_range):
        ...
        self.dates_a = pd.to_datetime(crypto.get_attention_df['Date'])
        ...
        self.train_set_a = self.get_train_set_a()
    ...
    def get_train_set(self):
        ...
        try:
            ...
            self.hist_range = [self.dates[0],self.dates[999]]
            return train_set
        
        if isinstance(self.hist_range,list) and len(self.hist_range)==2 \
    ...
    def get_train_set_a(self):
        ...
        try:
            ...
            self.hist_range = [self.dates_a[0],self.dates_a[999]]
            return train_set_a
        
        if isinstance(self.hist_range,list) and len(self.hist_range)==2 \
                        and start_date>=self.dates_a[0] and end_date<self.dates_a.iloc[-1]:
            start_idx,end_idx = self.dates_a[self.dates_a==start_date].index[0],self.dates_a[self.dates==end_date].index[0]
            train_set_a = self.prices[start_idx:end_idx]
        else:
            print('Input parameters were not valid. Default train set selected')
            train_set_a = self.attention[0:1000]
            self.hist_range = [self.dates_a[0],self.dates_a[999]]
